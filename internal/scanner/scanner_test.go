...
func TestScan_Clean(t *testing.T) {
	...
	report, err := scanner.Scan(db, scanner.Options{Dir: dir})
	require.NoError(t, err)
...
func TestScan_SymlinkSkipped(t *testing.T) {
	...
	db := mustDB(t, emptyLoader{})
	report, err := scanner.Scan(db, scanner.Options{Dir: dir, FollowLinks: false})
	require.NoError(t, err)
...
func TestScan_OversizedSkipped(t *testing.T) {
	...
	report, err := scanner.Scan(db, scanner.Options{Dir: dir, MaxFileSizeB: 0})
	require.NoError(t, err)
	...
	report2, err := scanner.Scan(db, scanner.Options{Dir: dir, MaxFileSizeB: 1})
	require.NoError(t, err)
